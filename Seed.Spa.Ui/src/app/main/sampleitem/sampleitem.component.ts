...
@Component({
    ...
    templateUrl: './sampleitem.component.html',
    styleUrls: ['./sampleitem.component.css'],
    host: {
        '[class.className]': '_showClassName',
        '[class]': '_classNames'
    }
})
export class SampleItemComponent extends ComponentBase implements OnInit, OnDestroy {
    ...
    constructor(private sampleItemService: SampleItemService, private router: Router, private ref: ChangeDetectorRef) {
        super();
        this.vm = null;
    }
    ...
    public onFilter(modelFilter: any) {
        modelFilter.queryOptimizerBehavior = "GRID_SampleItem".toUpperCase();
        this.sampleItemService.get(modelFilter).subscribe((result) => {
    ...
    public onExport() {
        this.sampleItemService.export(Object.assign(this.vm.modelFilter, { AttributeBehavior: "exportar" })).subscribe((result) => {
    ...
    public onCreate() {
        ...
        this.navigateStrategy(this.vm, this.saveModal, this.router, "/sampleitem/create");
    }
    ...
    public onEdit(model: any) {
        ...
        if (model)  {
            newModel = { id: model.sampleItemId }
        }

        if (!this.vm.navigationModal) {
            this.navigateStrategy(this.vm, this.editModal, this.router, "/sampleitem/edit/" + newModel.id);
        }
        else {
            this.sampleItemService.get(newModel).subscribe((result) => {
    ...
    public onDetails(model: any) {
        ...
        if (!this.vm.navigationModal) {
            this.navigateStrategy(this.vm, this.editModal, this.router, "/sampleitem/details/" + newModel.id);
        }
        else {
            this.sampleItemService.get(newModel).subscribe((result) => {
    ...
    public onPrint(model: any) {
        this.router.navigate(['/sampleitem/print', model.sampleItemId]);
    }
    ...
    public onDeleteConfimation(model: any) {
        ...
        var conf = GlobalService.operationExecutedParameters(
            ...
            () => {
                ...
                this.sampleItemService.delete(newModel).subscribe((result) => {
                    this.vm.filterResult = this.vm.filterResult.filter(function (model) {
                        return  model.sampleItemId != result.data.sampleItemId;
                    });
    ...
    public onPageChanged(pageConfig: any) {
        ...
        let modelFilter = this.sampleItemService.pagingConfig(this.vm.modelFilter, pageConfig);
        this.sampleItemService.get(modelFilter).subscribe((result) => {
            this.vm.filterResult = result.dataList;
            this.vm.summary = result.summary;
        });
    ...
    public onOrderBy(order: any) {
        ...
        let modelFilter = this.sampleItemService.orderByConfig(this.vm.modelFilter, order);
        this.sampleItemService.get(modelFilter).subscribe((result) => {
            this.vm.filterResult = result.dataList;
            this.vm.summary = result.summary;
        });
