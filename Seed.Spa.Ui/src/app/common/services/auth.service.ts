...
export class AuthService {
  ...
  public loginClientCredenciais(callback = null) {
    ...
    this.apiAuth.enableNotification(false).setResource("authApp", GlobalService.getEndPoints().AUTHAPI).post({
      ...
      ClientSecret: this._client_secret_cc,
      Scope: this._scope,
      process: 'AuthServices'
    }).subscribe(resultAuth => {
      CacheService.add(this._nameTokenAnonymous, resultAuth.data.accessToken, this._cacheType);
      if (callback)
        callback(resultAuth.data.accessToken);
